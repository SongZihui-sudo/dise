...
public:
  ...
  LogEvent(LogLevel::level in_level, uint32_t in_line, std::string in_file,
           uint64_t in_elapse, std::string in_threadName, uint32_t in_threadId,
           uint32_t in_fiberId, uint64_t in_time);
  ...
  std::ostringstream &get_formatted() {
  ...
  void set_file(std::string in_file) {
    this->m_file = in_file;
  } /* 设置文件名 */
  ...
  void set_formatted(std::string str) {
    this->m_formatted << str;
  } /* 设置格式化串 */
  ...
  void set_logName(std::string LogName) {
    this->m_name = LogName;
  } /* 设置日志名称 */
...
public:
  ...
  */
  void format(const char *pattern,
              std::initializer_list<std::string> arg); /* 类似与 printf */
...
public:
  StdLogAppender(LogLevel::level in_level) : LogAppender(in_level){};
...
public:
  FileLogAppender(LogLevel::level in_level) : LogAppender(in_level) {
...
public:
  ...
  Logger(LogAppender::ptr in_appender, std::string in_name,
         LogLevel::level in_level) {
    this->m_formatter.reset(new LogFormatter);
  ...
  */
  ...
  void set_appender(Appender flag,
                    LogLevel::level in_level = LogLevel::level::DOTKNOW) {
    switch (flag) {
#define XX(name, bit)                                                          \
  case Appender::bit:                                                          \
...
#undef XX
  ...
  Logger &operator<<(std::string str) {
    ...
    while (*temp) {
      if (flag) {
        ...
        switch (*temp) {
        case 'D':
          TimeItem::format(this->m_formatter->get_formatted()->get_formatted(),
                           nullptr);
          temp++;
          break;
        case 'n':
  ...
  */
  void format(const char *pattern, std::initializer_list<std::string> args) {
    ...
    this->log(this->m_formatter->get_formatted()->getLevel(),
              this->m_formatter->get_formatted());
    std::string temp =
        this->m_formatter->get_formatted()->get_formatted().str();
